...
public class SecurityConfig {
    ...
    public PublicKey publicKey(@Value("${jwt.keys.public}") String publicKey)
        ...
        byte[] public_bytes = decoder.decode(
    ...
    public PrivateKey privateKey(@Value("${jwt.keys.private}") String privateKey)
        ...
        byte[] private_bytes = decoder.decode(
        ...
        KeyFactory keyFactory = KeyFactory.getInstance("RSA");
        PKCS8EncodedKeySpec private_spec = new PKCS8EncodedKeySpec(private_bytes);
        return keyFactory.generatePrivate(private_spec);
    }
}
